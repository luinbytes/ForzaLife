...
impl Simulation {
    pub fn update(&mut self, telemetry: &Telemetry) -> LifeSnapshot {
        ...
        if elapsed_s > 0.0 && telemetry.current_engine_rpm > 100.0 {
            let rpm_ratio =
    ...
    pub fn refuel(&mut self, car_ordinal: i32) {
        self.vehicles.entry(car_ordinal).or_default().fuel_liters = DEFAULT_TANK_LITERS;
    }
...
fn snapshot(car_ordinal: i32, vehicle: &VehicleLife) -> LifeSnapshot {
    LifeSnapshot {
        ...
        fuel_percent: vehicle.fuel_liters / DEFAULT_TANK_LITERS,
